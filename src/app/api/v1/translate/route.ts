...
import { translateOne, translateRequestSchema } from "@/lib/translate/translate-service";
...
export const POST = createScopedHandler("normalize", async (request) => {
  ...
  const payload = translateRequestSchema.safeParse({
    ...raw,
  ...
  return NextResponse.json(await translateOne(payload.data));
});
