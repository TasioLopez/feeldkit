...
import { translateBatch, translateRequestSchema } from "@/lib/translate/translate-service";

const batchSchema = z.object({
  items: z.array(translateRequestSchema).min(1).max(100),
});
...
export const POST = createScopedHandler("normalize", async (request) => {
  ...
  return NextResponse.json({ items: await translateBatch(payload.data.items) });
});
