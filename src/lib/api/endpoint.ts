...
import { withApiScope, withApiScopes } from "@/lib/auth/require-api-scope";
...
export function createMultiScopedHandler(
  ...
  return async (request: Request) => {
    ...
    const url = new URL(request.url);
    const requestId = request.headers.get("x-request-id") ?? undefined;
    const auth = await withApiScopes(request, scopes);
    if (!auth.ok) {
      logApiRequest({
        requestId,
        method: request.method,
    ...
    void logUsageEvent({
      ...
      apiKeyId: auth.apiKey.id,
      endpoint: url.pathname,
      fieldKey: url.searchParams.get("fieldKey") ?? null,
    });
    logApiRequest({
      requestId,
      method: request.method,
