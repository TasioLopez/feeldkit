...
async function writeReport(packs: Awaited<ReturnType<typeof buildFullV1Packs>>, summary?: Record<string, unknown>) {
...
async function run() {
  ...
  if (args.dryRun) {
    await writeReport(packs, { mode: "dry-run" });
  ...
  await writeReport(packs, { ...summary, crosswalks: crosswalkCount, mode: "apply" });
  console.log(
    `Full V1 import complete. packs=${summary.packs} field_types=${summary.fieldTypes} values=${summary.fieldValues} aliases=${summary.aliases} crosswalks=${crosswalkCount}`,
  );
}
